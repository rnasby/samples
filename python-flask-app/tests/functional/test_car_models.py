...
def test_create_car_model(test_client):
    ...
    reply = test_client.post(f'{API}', json={"name": "Mustang", "make_id": 1, "year": 1979, "price": 6700.00})
...
def test_get_car_model_list(test_client):
    reply = test_client.post(f'{API}', json={"name": "Cougar", "make_id": 1, "year": 1980, "price": 9800.00})
    assert reply.status_code == HTTPStatus.CREATED
...
def test_update_car_model(test_client):
    reply = test_client.post(f'{API}', json={"name": "Bogus", "make_id": 1, "year": 1900, "price": 200.00})
    assert reply.status_code == HTTPStatus.CREATED

    reply = test_client.put(f'{API}/3', json={"name": "Bogus2", "make_id": 2, "year": 1910, "price": 1.75})
