...
def test_create_car_part(test_client):
    ...
    reply = test_client.post(f'{BASE}', json={"name": "Alternator", "price": 500.50 })
    assert reply.status_code == HTTPStatus.CREATED
...
def test_get_car_part_list(test_client):
    reply = test_client.post(f'{BASE}', json={"name": "Motor", "price": 9500.50 })
    assert reply.status_code == HTTPStatus.CREATED
...
def test_update_car_part(test_client):
    reply = test_client.post(f'{BASE}', json={"name": "Bogus", "price": 0.50})
    assert reply.status_code == HTTPStatus.CREATED

    reply = test_client.put(f'{BASE}/3', json={"name": "Bogus2", "price": 0.75})
